...
void process_packet(unsigned char *pkt, int pkt_len)
{
	...
	if (tcph->syn && (!tcph->ack)) {	// SYN packet, send SYN+ACK
		...
		if ((*tcp_payload != 'G') || (*(tcp_payload + 1) != 'E') || (*(tcp_payload + 2) != 'T') || (*(tcp_payload + 3) != ' ')) {
#ifdef DEBUG
		...
		*(tcp_payload + i) = 0;
...
#endif
		if (strncmp((char *)url, "/library/test/success.html", 26) == 0)
			return;
		if (strncmp((char *)url, "/wangwang/get_task.php", 22) == 0)
			return;
		if (strncmp((char *)url, "/list=sys_hqEtagMode", 20) == 0)
			return;
		if (strncmp((char *)url, "/editor/guy.gif", 15) == 0)
			return;
		if (strncmp((char *)url, "/webservice.php", 15) == 0)
			return;
		if (strncmp((char *)url, "/sync/get?uid=", 14) == 0)
			return;
		if (strncmp((char *)url, "/announce.php", 13) == 0)
			return;
		if (strncmp((char *)url, "/etag.php?rn=", 13) == 0)
			return;
		if (strncmp((char *)url, "/generate_204", 13) == 0)
			return;
		if (strncmp((char *)url, "/player.htm?", 13) == 0)
			return;
		if (strncmp((char *)url, "/getts.json", 11) == 0)
			return;
		if (strncmp((char *)url, "/res/static", 11) == 0)
			return;
		if (strncmp((char *)url, "/din.aspx?", 10) == 0)
			return;
		if (strncmp((char *)url, "/comet_get", 10) == 0)
			return;
		if (strncmp((char *)url, "/announce?", 10) == 0)
			return;
		if (strncmp((char *)url, "/v1/route", 9) == 0)
			return;
		if (strncmp((char *)url, "/profile/", 9) == 0)
			return;
		if (strncmp((char *)url, "/1.html?", 8) == 0)
			return;
		if (strncmp((char *)url, "/?qt=rg", 7) == 0)
			return;
		if (strncmp((char *)url, "/api/", 5) == 0)
			return;
		if (strncmp((char *)url, "/p2p?", 5) == 0)
			return;
		if (strcmp((char *)url, "/abc") == 0)
			return;

		if (strcmp((char *)host, PH) == 0)
			return;
		if (strcmp((char *)host, "mirrors.hust.edu.cn") == 0)
			return;
		if (strcmp((char *)host, "sdup.360.cn") == 0)
			return;
		if (strcmp((char *)host, "update.360safe.com") == 0)
			return;
		if (strcmp((char *)host, "jipiao.jd.com") == 0)
			return;
		if (strcmp((char *)host, "jipiao.kuxun.cn") == 0)
			return;
		if (strcmp((char *)host, "flight.elong.com") == 0)
			return;
		if (strcmp((char *)host, "static.tieba.baidu.com") == 0)
			return;
		if (strcmp((char *)host, "message.tieba.baidu.com") == 0)
			return;

		if ((strncmp((char *)agent, "Wget", 4) == 0) || (strncmp((char *)agent, "Debian APT-HTTP", 15) == 0)
		    ) {
			return;
		}
#if	1
		...
		if (strlen((char *)host) + strlen((char *)url) < 200)
			ntcp_payload_len = snprintf((char *)ntcp_payload, 1200, http_redirect_content1, host, url, IPQUAD(ip->daddr));
		else
			ntcp_payload_len = snprintf((char *)ntcp_payload, 1200, http_redirect_content2);
#ifdef DEBUG
...
#ifdef DEBUG
	fprintf(stderr, "len %d iphdr %d %u.%u.%u.%u ->", pdata_len, iphdrp->ihl << 2, IPQUAD(iphdrp->saddr));
	fprintf(stderr, " %u.%u.%u.%u", IPQUAD(iphdrp->daddr));
	fprintf(stderr, "\n");
#endif
	if ((iphdrp->version == 4)
