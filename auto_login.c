...
MYSQL_RES *ExecSQL(char *sql, int haveresult)
{
	...
	if (mysql_query(mysql, sql)) {
		fprintf(stderr, "mysql_querying error: %s\n", sql);
		return NULL;
	}
	if (haveresult) {
...
void AddNewUser(char *ip)
{
	...
	snprintf(buf, MAXLEN, "/usr/sbin/ipset add -exist newuser %s,%c%c:%c%c:%c%c:%c%c:%c%c:%c%c timeout %d",
		 ip, MAC[0], MAC[1], MAC[2], MAC[3], MAC[4], MAC[5], MAC[6], MAC[7], MAC[8], MAC[9], MAC[10], MAC[11], 600);
	system(buf);
...
void IPOnline(char *ip, int timespan)
{
	...
	snprintf(buf, MAXLEN, "/usr/sbin/ipset add -exist user %s,%c%c:%c%c:%c%c:%c%c:%c%c:%c%c timeout %d",
		 ip, MAC[0], MAC[1], MAC[2], MAC[3], MAC[4], MAC[5], MAC[6], MAC[7], MAC[8], MAC[9], MAC[10], MAC[11], timespan);
	system(buf);
	snprintf(buf, MAXLEN, "insert into IPMACPhone values('%s', '%s','%s',now(), date_add(now(), interval %d second))", ip, MAC, PHONE, timespan);
	ExecSQL(buf, 0);
...
void process_auto_login(char *ip)
{
	...
	if (row) {
		...
		snprintf(buf, MAXLEN, "insert into Log values('%s','%s','%s',now(),'VIP %s auto_login online 7 day')", ip, MAC, PHONE, row[1]);
		ExecSQL(buf, 0);
#ifdef DEBUG
	...
	if (row) {
		snprintf(PHONE, 12, row[0]);
		snprintf(buf, MAXLEN, "insert into Log values('%s','%s','%s',now(),'auto_login online %s sec')", ip, MAC, PHONE, row[1]);
		ExecSQL(buf, 0);
#ifdef DEBUG
...
#ifdef DEBUG
	fprintf(stderr, "len %d iphdr %d %s ->", pdata_len, iphdrp->ihl << 2, inet_ntoa(*(struct in_addr *)&iphdrp->saddr));
	fprintf(stderr, " %s", inet_ntoa(*(struct in_addr *)&iphdrp->daddr));
	fprintf(stderr, "\n");
#endif
	if (iphdrp->version == 4)
