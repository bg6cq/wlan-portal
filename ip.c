...
MYSQL *ConnectDB(void)
{
	...
	if (mysql_real_connect(mysql, AUTHDBHOST, AUTHDBUSER, AUTHDBPASSWD, AUTHDB, AUTHDBPORT, AUTHDBSOCKPATH, 0) == NULL)
...
char *FilterLine(char *line)
{
	...
	while (*p) {
		s = "USERIP";
		if ((ps = strstr(p, s))) {
			strncpy(buf + i, p, ps - p);
			i += ps - p;
			strcpy(buf + i, remote_addr());
			i += strlen(remote_addr());
			p = ps + strlen(s);
			continue;
		}
		s = "MACADDR";
...
void IPOnline(int timespan)
{
	...
	snprintf(buf, MAXLEN, "/usr/sbin/ipset add -exist user %s,%c%c:%c%c:%c%c:%c%c:%c%c:%c%c timeout %d",
		 remote_addr(), MAC[0], MAC[1], MAC[2], MAC[3], MAC[4], MAC[5], MAC[6], MAC[7], MAC[8], MAC[9], MAC[10], MAC[11], timespan);
	system(buf);
	snprintf(buf, MAXLEN, "insert into IPMACPhone values('%s', '%s','%s',now(), date_add(now(), interval %d second))", remote_addr(), MAC, PHONE, timespan);
	ExecSQL(buf, 0);
	url = GetValue("url");
	if (url && (*url) && (strcmp(url, "URL") != 0)) {
		...
		printf("<script language=\"javascript\" type=\"text/javascript\">" "window.location.href=\"%s\";</script>\n", url);
...
void Stage0(void)
{
	...
	if (row) {
		...
		snprintf(buf, MAXLEN, "insert into Log values('%s','%s','%s',now(),'VIP %s auto online 7 day')", remote_addr(), MAC, PHONE, row[1]);
		ExecSQL(buf, 0);
		IPOnline(7 * 24 * 3600);
	}
	...
	if (row) {
		snprintf(PHONE, 12, row[0]);
		snprintf(buf, MAXLEN, "insert into Log values('%s','%s','%s',now(),'auto online %s sec')", remote_addr(), MAC, PHONE, row[1]);
		ExecSQL(buf, 0);
		IPOnline(atoi(row[1]));
	}
...
void Stage1()			// sendsms, dispay input page
{
	...
	ExecSQL(buf, 0);
	snprintf(buf, MAXLEN,
		 "php /usr/src/sendsms/sendsms.php %s \"%s是您在中国科大访客WLAN密码，一周内都可以使用本密码登录，请保留本短信。\" 2>/dev/null",
		 PHONE, pass);
	fp = popen(buf, "r");
	if (fp == NULL) {
		snprintf(buf, MAXLEN, "insert into Log values('%s','%s','%s',now(),'send pass error')", remote_addr(), MAC, PHONE);
		ExecSQL(buf, 0);
		DisplayStage('0', "密码发送失败", 1);
	}
	fgets(buf, MAXLEN, fp);
	if (strncmp(buf, "OK", 2) == 0) {
		snprintf(buf, MAXLEN, "insert into Log values('%s','%s','%s',now(),'send pass ok')", remote_addr(), MAC, PHONE);
		ExecSQL(buf, 0);
		DisplayStage('1', "请输入手机上收到的密码", 0);
	} else {
		char tmp[MAXLEN];
		strncpy(tmp, buf, MAXLEN);
		snprintf(buf, MAXLEN, "insert into Log values('%s','%s','%s',now(),'send pass error %s')", remote_addr(), MAC, PHONE, tmp);
		ExecSQL(buf, 0);
